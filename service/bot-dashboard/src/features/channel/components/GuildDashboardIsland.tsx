    ...
    <>
      ...
      <div className="overflow-hidden rounded-2xl bg-surface-container-low">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left" aria-label={translations.table.title}>
            <thead>
              <tr className="bg-surface-container-high text-[10px] font-bold uppercase tracking-[0.12em] text-on-surface-variant">
                <th scope="col" className="px-6 py-4 sm:px-8">{translations.table.channelName}</th>
                <th scope="col" className="hidden px-6 py-4 sm:table-cell sm:px-8">{translations.table.language}</th>
                <th scope="col" className="hidden px-6 py-4 md:table-cell sm:px-8">{translations.table.members}</th>
                <th scope="col" className="hidden px-6 py-4 lg:table-cell sm:px-8">{translations.table.status}</th>
                <th scope="col" className="px-6 py-4 text-right sm:px-8">{translations.table.actions}</th>
              </tr>
            </thead>
            <tbody>
              {channels.map((ch, i) => (
                ...
                >
                  <td className="px-6 py-4 sm:px-8">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-vspo-purple/10 text-vspo-purple">
                        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-on-surface">{ch.channelName}</p>
                        <p className="text-[10px] text-on-surface-variant/60">{ch.channelId}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="hidden px-6 py-4 text-xs font-medium text-on-surface-variant md:table-cell sm:px-8">
                    {ch.memberType === "custom" ? (() => {
                      ...
                      if (resolved.length === 0) return memberTypeLabels[ch.memberType];
                      const overflow = (ch.customMembers?.length ?? 0) - MAX_AVATARS;
                      return (
                        <div className="flex items-center -space-x-2">
                          {resolved.slice(0, MAX_AVATARS).map((c) => (
                            <div key={c.id} className="relative h-7 w-7 shrink-0 rounded-full ring-2 ring-surface" title={c.name}>
                              {c.thumbnailUrl ? (
                                <img src={c.thumbnailUrl} alt={c.name} width={28} height={28} className="h-7 w-7 rounded-full object-cover" loading="lazy" />
                              ) : (
                                <div className="flex h-7 w-7 items-center justify-center rounded-full bg-vspo-purple/20 text-[10px] font-bold text-vspo-purple" role="img" aria-label={c.name}>{c.name.charAt(0)}</div>
                              )}
                            </div>
                          ))}
                          {overflow > 0 && (
                            <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-surface-container-highest text-[10px] font-bold text-on-surface-variant ring-2 ring-surface">+{overflow}</div>
                          )}
                        </div>
                      );
                  ...
                  <td className="hidden px-6 py-4 lg:table-cell sm:px-8">
                    {ch.enabled ? (
                      <div className="flex items-center gap-2">
                        <div className="relative flex h-2 w-2"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-success opacity-75" /><span className="relative inline-flex h-2 w-2 rounded-full bg-success shadow-sm shadow-success/50" /></div>
                        <span className="text-[10px] font-bold uppercase tracking-wider text-success">{translations.table.active}</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <span className="inline-flex h-2 w-2 rounded-full bg-on-surface-variant/40" />
                        <span className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant/60">{translations.table.paused}</span>
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4 sm:px-8">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      >
                        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button
                        type="button"
                        onClick={() => openDeleteDialog({ channelId: ch.channelId, channelName: ch.channelName })}
                      ...
                      >
                        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      </button>
                    </div>
                  </td>
                </tr>
        ...
        {channels.length === 0 && (
          <div className="flex flex-col items-center gap-3 p-12 text-center">
            <svg className="h-12 w-12 text-on-surface-variant/30" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
            </svg>
            <p className="text-sm text-on-surface-variant">{translations.table.empty}</p>
            <button type="button" onClick={openAddModal} className="mt-2 inline-flex items-center gap-1.5 text-sm font-medium text-vspo-purple transition-colors hover:text-vspo-purple/80">
